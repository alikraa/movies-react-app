...
function FilmsList() {
  ...
  const filmsList = showMovies(currentFilmsList, currentReleaseYear, genres);

  useEffect(() => {
    if (filmsList) {
      const lastPageNumber = Math.ceil(
        filmsList.length / VALUES.displayedMovies
      );
      dispatch(savePageNumberCreator(ACTIONS.END_PAGE, lastPageNumber));
    }
  });

  const newMoviesList = sliceList(filmsList, firstPageNumber);
